...
% observer    = uavObserverNumDif(Param, measurments);
observer    = uavObserverDirtyDif(Param, measurments);
controller  = uavControllerPID(Param);
...
while t < Param.t_end
    ...
    while t <= t_next_plot
        ...
        % Inputs
        [input, theta_r] = controller.update(dynamics.states, reference);
    ...
    estimate   = states_hat;
    references = [h_r;z_r;theta_r; NaN(3,1)];
